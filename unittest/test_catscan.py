...
class TestCatScan(TestCase):
    ...
    def test_add_options(self):
        ...
        self.assertDictEqual({"smaller": "300", "max_age": "45"}, self.catscan.options)
    ...
    def test_add_namespace(self):
        ...
        self.assertDictEqual({"ns[0]": "1", "ns[2]": "1", "ns[6]": "1", "ns[10]": "1"}, self.catscan.options)

    def test_activate_redirects(self):
        self.catscan.activate_redirects()
        self.assertDictEqual({"show_redirects": "yes"}, self.catscan.options)

    def test_deactivate_redirects(self):
        self.catscan.deactivate_redirects()
        self.assertDictEqual({"show_redirects": "no"}, self.catscan.options)

    def test_last_change_before(self):
        self.catscan.last_change_before(1234, 1, 1, 2, 2, 42)
        self.assertDictEqual({"before": "12340101020242"}, self.catscan.options)

    def test_last_change_after(self):
        self.catscan.last_change_after(1234, 1, 1, 2, 2, 42)
        self.assertDictEqual({"after": "12340101020242"}, self.catscan.options)

    def test_max_age(self):
        self.catscan.max_age(1234)
        self.assertDictEqual({"max_age": "1234"}, self.catscan.options)

    def test_only_new(self):
        self.catscan.only_new()
        self.assertDictEqual({"only_new": "1"}, self.catscan.options)

    def test_smaller_then(self):
        self.catscan.smaller_then(42)
        self.assertDictEqual({"smaller": "42"}, self.catscan.options)

    def test_larger_then(self):
        self.catscan.larger_then(42)
        self.assertDictEqual({"larger": "42"}, self.catscan.options)

    def test_get_wikidata(self):
        self.catscan.get_wikidata()
        self.assertDictEqual({"get_q": "1"}, self.catscan.options)

    def test_set_logic_complete(self):
        self.catscan.set_logic(log_and = True, log_or = True)
        self.assertDictEqual({"comb[subset]": "1", "comb[union]": "1"}, self.catscan.options)

    def test_set_logic_only_and(self):
        self.catscan.set_logic(log_and = True)
        self.assertDictEqual({}, self.catscan.options)

    def test_set_logic_only_or(self):
        self.catscan.set_logic(log_or = True)
        self.assertDictEqual({"comb[union]": "1"}, self.catscan.options)
    ...
    def test_construct_templates(self):
        ...
        self.assertEqual(str(self.catscan),
                         'http://tools.wmflabs.org/catscan3/catscan2.php?language=de&project=wikisource&templates_yes=yes1%0D%0Ayes2&templates_any=any1%0D%0Aany2%0D%0Aany3&templates_no=no1%0D%0Ano2&format=json&doit=1')
    ...
    def test_construct_outlinks(self):
        ...
        self.assertEqual(str(self.catscan),
                         'http://tools.wmflabs.org/catscan3/catscan2.php?language=de&project=wikisource&outlinks_yes=yes1%0D%0Ayes2&outlinks_any=any1%0D%0Aany2%0D%0Aany3&outlinks_no=no1%0D%0Ano2&format=json&doit=1')


    def test_construct_options(self):
        self.catscan.options = {"max_age": "1234",
                                 "get_q": "1",
                                 "show_redirects": "yes"}
    ...
    def test_construct_string(self):
        ...
        self.assertEqual(str(self.catscan),
                         'http://tools.wmflabs.org/catscan3/catscan2.php?language=en&project=wikipedia&categories=test&format=json&doit=1')
        # only a negative category
        self.catscan.categories = {"positive": [], "negative": []}
        self.catscan.add_negative_category('test')
        self.assertEqual(str(self.catscan),
                         'http://tools.wmflabs.org/catscan3/catscan2.php?language=en&project=wikipedia&negcats=test&format=json&doit=1')
        # only a option
        self.catscan.categories = {"positive": [], "negative": []}
        self.catscan.add_options({"max_age": '10'})
        self.assertEqual(str(self.catscan),
                         'http://tools.wmflabs.org/catscan3/catscan2.php?language=en&project=wikipedia&max_age=10&format=json&doit=1')

    @httpretty.activate
    def test_do_positive(self):
        httpretty.register_uri(httpretty.GET,
                               'http://tools.wmflabs.org/catscan3/catscan2.php?language=de&project=wikisource&categories=Autoren&get_q=1&show_redirects=no&ns[0]=1&max_age=48&format=json&doit=1',
                               status=200,
